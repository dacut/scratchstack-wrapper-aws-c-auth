...
fn main() {
    ...
    let aws_c_http_include_dir =
        PathBuf::from(var("DEP_AWS_C_HTTP_INCLUDE").expect("DEP_AWS_C_HTTP_INCLUDE not set"));
    let aws_c_http_include_dir_str = aws_c_http_include_dir.to_string_lossy();
    let aws_c_io_include_dir =
        PathBuf::from(var("DEP_AWS_C_IO_INCLUDE").expect("DEP_AWS_C_IO_INCLUDE not set"));
    let aws_c_io_include_dir_str = aws_c_io_include_dir.to_string_lossy();
    let aws_c_sdkutils_include_dir =
        PathBuf::from(var("DEP_AWS_C_SDKUTILS_INCLUDE").expect("DEP_AWS_C_SDKUTILS_INCLUDE not set"));
    let aws_c_sdkutils_include_dir_str = aws_c_sdkutils_include_dir.to_string_lossy();

    let src_include_dir = root.join("include");
    let dst_include_dir = out_dir.join("include");
    let src_lib_include_dir = src_include_dir.join("aws").join("auth");
    let dst_lib_include_dir = dst_include_dir.join("aws").join("auth");
    ...
    let mut builder = builder()
        ...
        .clang_arg(format!("-I{aws_c_http_include_dir_str}"))
        .clang_arg(format!("-I{aws_c_io_include_dir_str}"))
        .clang_arg(format!("-I{aws_c_sdkutils_include_dir_str}"))
        .derive_debug(true)
        .derive_default(true)
    ...
    if n_includes == 0 {
        panic!("No header files found in include/aws/auth");
    }
